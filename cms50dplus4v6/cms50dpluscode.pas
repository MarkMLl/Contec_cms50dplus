...
type
  ...
  TCMS50DPlusForm = class(TForm)
    ...
    procedure MenuItemRunPlaybackClick(Sender: TObject);
    procedure MenuItemRunLiveClick(Sender: TObject);
    procedure MenuItemRunStopClick(Sender: TObject);
  strict private
    widthTweak, heightTweak: integer;
    procedure MenuItemRunStartCommon(Sender: TObject; op: integer);
  ...
  protected
    ...
    *)
    procedure ReadingLiveTxt(const time, spo2, bpm, sig, ir, vis, status: string15);
    ...
    *)
    procedure ReadingLiveBin(timestamp: TDateTime; ir, vis, spo2, bpm: double);
  public
...
type
  ...
  public
    constructor Create(CreateSuspended: Boolean; mySerialHandle: TSerialHandle;
                                                                op: TOperation);
    procedure writerPlaybackShim;
    procedure writerLiveShim;
  end;
...
var
  ...
  irBin, visBin, spo2Bin, bpmBin: double;
  nowt: TDateTime;
...
(* Provided that it is called via Synchronize(), this executes in the context of
  the main (GUI) thread.
*)
procedure TcommsThread.writerPlaybackShim;

begin
  if Pos('OK', extra) > 0 then begin
    CMS50DPlusForm.ReadingLiveTxt(time, spo2, bpm, '', '', '', extra);
    CMS50DPlusForm.ReadingLiveBin(nowt, -99, -99, spo2Bin, bpmBin)
  end;
...
end { TcommsThread.writerPlaybackShim } ;
...
(* This is called by the datacomms loop in the datacomms thread to handle each
  ...
  (immediately above).
*)
procedure writerPlayback(const s: string);

var
  baseTime: string;
  secs, mins, hrs, baseSecs: integer;

begin
  if Pos('|', s) < 1 then begin         (* Initialise any counters etc.         *)
  end else begin
    ...
    time := baseTime;
    nowt := 0.0 + baseSecs / SecsPerDay;
    spo2 := Trim(ExtractWord(3, s, ['|']));
...
(* Provided that it is called via Synchronize(), this executes in the context of
  the main (GUI) thread.
*)
procedure TcommsThread.writerLiveShim;

begin
  CMS50DPlusForm.ReadingLiveTxt(time, spo2, bpm, sig, ir, vis, extra);
  CMS50DPlusForm.ReadingLiveBin(Now(), irBin, visBin, spo2Bin, -99);
...
(* an exception which needs to be caught and rendered harmless.                 *)
  ...
  if Pos('OK', extra) = 0 then
    CMS50DPlusForm.MenuItemRunStopClick(nil) (* Ready for repeat reading        *)
end { TcommsThread.writerLiveShim } ;
...
*)
procedure writerLive(const s: string);
...
begin
  if Pos('|', s) < 1 then begin         (* Initialise any counters etc.         *)
    ...
    spo2Bin := StrToFloat(spo2);
    CMS50DPlusForm.CommsThread.Synchronize(@CMS50DPlusForm.CommsThread.writerLiveShim)
  end
end { writerLive } ;


(* Main datacomms code. Because RunConsoleAppLive() may run either in the context
...
begin
  case operation of
    opLive:     rslt := RunConsoleApp2(serialHandle, operation, stopLoop, @writerLive);
    opPlayback: rslt := RunConsoleApp2(serialHandle, operation, stopLoop, @writerPlayback)
  otherwise
    rslt := 0
  end;
  case rslt of
  ...
  if (messageStr <> '') and not stopLoop then
    Synchronize(@msgShim);
  messageStr := '';
  CMS50DPlusForm.fCommsThread := nil
...
(* /dev/ttyUSB0.                                                                *)
  ...
  try
    ...
    MenuItemRunPlayback.Enabled := MenuItemRun.Enabled
  finally
...
(* The integer parameter here avoids a circular dependency between the units.
*)
procedure TCMS50DPlusForm.MenuItemRunStartCommon(Sender: TObject; op: integer);
...
{$endif UNIX }
  ...
  if portHandle <> InvalidSerialHandle then begin
    case TOperation(op) of
      opLive:     GroupBoxReading.Caption := 'Reading';
      opPlayback: GroupBoxReading.Caption := 'Playing back'
    otherwise
      halt
    end;
    MenuItemRunStart.Enabled := false;
    MenuItemRunPlayback.Enabled := false;
...
(* naughty and assume this is done correctly.                                   *)
    ...
    fcommsThread := TcommsThread.Create(false, portHandle, TOperation(op));
    MenuItemRunStop.Enabled := true
  end
end { TCMS50DPlusForm.MenuItemRunStartCommon } ;


procedure TCMS50DPlusForm.MenuItemRunPlaybackClick(Sender: TObject);

begin
  MenuItemRunStartCommon(Sender, Ord(opPlayback))
end { TCMS50DPlusForm.MenuItemRunPlaybackClick } ;


procedure TCMS50DPlusForm.MenuItemRunLiveClick(Sender: TObject);

begin
  MenuItemRunStartCommon(Sender, Ord(opLive))
...
(* There's a very slight risk of a race condition here, if the thread releases  *)
(* its own resources between the Assigned() and Terminate. For that reason I've *)
(* put an exception handler around it.                                          *)

  try
    if Assigned(fcommsThread) then
      fcommsThread.Terminate
  except
  end;
  ...
  MenuItemRunPlayback.Enabled := true
...
(* Output the values read from the meter in text form. This is called, via
  Synchronize(), by the background communications thread.
*)
procedure TCMS50DPlusForm.ReadingLiveTxt(const time, spo2, bpm, sig, ir, vis, status: string15);
...
end { TCMS50DPlusForm.ReadingLiveTxt } ;


(* Output the values read from the meter in binary form for the chart etc.
  This is called, via Synchronize(), by the background communications thread.
*)
procedure TCMS50DPlusForm.ReadingLiveBin(timestamp: TDateTime; ir, vis, spo2, bpm: double);

var
  i: integer;

begin
  Chart1LineSeries2.Active := ir >= 0;
  Chart1LineSeries3.Active := vis >= 0;
  Chart1LineSeries4.Active := bpm >= 0;
  if Chart1LineSeries1.Count = 0 then   (* Make startup a bit more predictable  *)
    for i := -99 to 0 do begin
      ...
      Chart1LineSeries4.AddXY(Now() + (0.5 * i / SecsPerDay), bpm)
    end;
